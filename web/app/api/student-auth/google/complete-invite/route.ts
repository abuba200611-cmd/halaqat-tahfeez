...
export async function POST(request: Request) {
  ...
  if (!pending) {
    // هذا الفرع لا يعني بالضرورة أن ١٠ دقائق مرّت فعلاً: الكوكي أحادية
    // الاستخدام (انظر التعليق أعلاه) فتُمسَح أيضاً بعد أي محاولة سابقة
    // فاشلة (رمز دعوة غلط) بنفس التحقق — حتى لو بعدها بثوانٍ. لا نملك
    // إشارة تميّز "انتهت فعلاً" عن "استُهلكت بمحاولة سابقة"، فالرسالة
    // تذكر الاحتمالين بدل الجزم بأحدهما (كان يُجزم بـ"انتهت" فقط، وهو
    // تصنيف غير دقيق في حالة الاستهلاك بمحاولة سابقة).
    return Response.json(
      {
        error:
          "لم يعد تحقق جوجل صالحاً (إمّا انتهت مهلته أو استُهلك بمحاولة سابقة برمز غير صحيح) — سجّل الدخول بحساب جوجل من جديد",
      },
      { status: 401 },
    );
  }
